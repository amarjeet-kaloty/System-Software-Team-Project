...
char identifiers_list[20000][IDENTIFIER_MAX_LENGTH];
int  identifiers_ints[20000];
...
void main(int print){
    ...
    for (i=0; i < input_length; i++){
        ...
        { 
			...
			//Check for != and <=
			if(temp == lessym){
				secondSymbol = characterToToken(inputArray[i+1]);
        ...
        if (inputArray[i] >= '0' && inputArray[i] <= '9')
        {
            ...
            for (j= i; j< NUM_MAX_LENGTH+i+1 ; j++)
            {
                ...
                //If digits include a letter, declare it as invalid
