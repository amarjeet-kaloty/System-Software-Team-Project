...
void main(int flag){
  ...
  int i;
   ...
   //Open lexeme list
  PCGinput = fopen("lexicalOutput.txt", "r");
  ...
  int x = 0;
  while( !( codearray[x].op == 0 && codearray[x].l == 0 && codearray[x].m == 0 ) ){
    if(flag)
        printf("%d %d %d\n", codearray[x].op, codearray[x].l, codearray[x].m );
    fprintf(fout, "%d %d %d\n", codearray[x].op, codearray[x].l, codearray[x].m );
    x++;
  }
